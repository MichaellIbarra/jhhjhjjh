...
export default function TeacherSettingsPage() { // Renamed
  ...
  useEffect(() => {
    if (!isAuthLoading && (!currentUser || currentUser.role === 'superuser')) {
        if (currentUser && currentUser.role === 'superuser') {
            router.push('/admin/settings'); // Redirect superusers
        } else {
            router.push('/login');
        }
    }
    ...
    <DashboardLayout>
      <div className="max-w-3xl mx-auto space-y-8">
        <div className="flex items-center gap-3 mb-8">
            ...
            <div>
            <h1 className="text-3xl font-bold text-foreground">Configuración del Maestro</h1>
