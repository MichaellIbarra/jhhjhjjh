...
export default function AdminProfilePage() { // Renamed
  ...
  useEffect(() => {
    if (!isAuthLoading && (!currentUser || currentUser.role !== 'superuser')) {
        if (currentUser && currentUser.role === 'normal') {
            router.push('/teacher/profile'); // Redirect normal users to their profile
        } else {
            router.push('/login');
        }
    }
  ...
  // This check is redundant due to useEffect but good for explicit rendering control
        ...
        <Card className="shadow-lg rounded-xl overflow-hidden">
          <CardHeader className="items-center text-center bg-muted/30 p-8">
            ...
            <CardDescription className="text-lg text-muted-foreground">
              Superusuario (Administrador Global)
            </CardDescription>
          </CardHeader>
          <CardContent className="p-6 md:p-8 space-y-6">
            ...
            <div className="flex items-start space-x-4 p-4 bg-background border border-border rounded-lg shadow-sm">
              ...
              <div>
                ...
                <p className="font-semibold text-lg text-foreground">Superusuario</p>
              </div>
            </div>
             <div className="flex items-start space-x-4 p-4 bg-background border border-border rounded-lg shadow-sm">
